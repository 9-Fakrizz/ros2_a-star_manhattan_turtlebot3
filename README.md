...
## 🧭 How A\* Works in This Code

The A\* algorithm in this project (with the core logic likely residing in `move_to_target.py` within the `robot401` package) is used to find the **shortest path** from the robot's starting position to the goal on a **Grid Map**.

A\* calculates the **Total Estimated Cost ($f$)** for traversing any cell $n$ on the map using the following formula:

$$f(n) = g(n) + h(n)$$

| Symbol | Name in Concept | Meaning in Code |
| :---: | :--- | :--- |
| $\mathbf{g(n)}$ | **Movement Cost (Actual Cost Used)** | This is the **actual accumulated distance** from the Start point to the current cell $n$. |
| $\mathbf{h(n)}$ | **Heuristic Cost (Estimated Cost)** | This is the **estimated distance** from the current cell $n$ to the Goal point. This code uses **Manhattan Distance**. |
| $\mathbf{f(n)}$ | **Total Cost (Estimated Total Cost)** | This value is used to prioritize which cell to explore. The cell with the lowest $f(n)$ value is chosen next. |
---

## 🛠️ Calculation Details

### 1. Actual Movement Cost $\mathbf{g(n)}$

The cumulative cost, $g(n)$, increases based on the type of movement taken to reach the current cell:

* **Orthogonal Movement** (Up, Down, Left, Right): **Cost $1.0$**
* **Diagonal Movement**: **Cost $\sqrt{2} \approx 1.414$**

### 2. Heuristic Cost $\mathbf{h(n)}$ (Manhattan Distance)

The code uses the **Manhattan Distance** formula for the estimated cost, which measures distance only along the grid axes (X and Y):
...
Let's use the assumed grid parameters:

* **Start Cell (S):** $(x_S, y_S) = (1, 1)$
* **Goal Cell (G):** $(x_G, y_G) = (5, 4)$
* **A\* Function:** $f(n) = g(n) + h(n)$

### Step 1: Calculate Cost at the Start Cell $\mathbf{(1, 1)}$

* **$\mathbf{g(S)}$:** $0$ (No movement yet)
* **$\mathbf{h(S)}$:** $|5-1| + |4-1| = 4 + 3 = 7$
* **$\mathbf{f(S)}$:** $0 + 7 = 7$

### Step 2: Evaluate Neighbor Cells (Round 1)

A\* calculates $f(n)$ for all 8 neighbors. The $g(n)$ is the cost from the starting cell $S(1, 1)$ to that neighbor.

| **Neighbor (n)** | **Position (x, y)** | **$g(n)$ (Accumulated Cost)** | **$h(n)$ (Manhattan Distance)** | **$f(n) = g(n) + h(n)$** |
| :---: | :---: | :---: | :---: | :---: |
| **NE** | **(2, 2)** | **$1.414$** ($\sqrt{2}$) | **$5$** | **$\mathbf{6.414}$ (Selected)** |
| N | (1, 2) | $1.0$ | $6$ | $\mathbf{7.0}$ |
| E | (2, 1) | $1.0$ | $6$ | $\mathbf{7.0}$ |
| NW | (0, 2) | $1.414$ ($\sqrt{2}$) | $7$ | $\mathbf{8.414}$ |
| SE | (2, 0) | $1.414$ ($\sqrt{2}$) | $7$ | $\mathbf{8.414}$ |
| S | (1, 0) | $1.0$ | $8$ | $\mathbf{9.0}$ |
| W | (0, 1) | $1.0$ | $8$ | $\mathbf{9.0}$ |
| SW | (0, 0) | $1.414$ ($\sqrt{2}$) | $9$ | $\mathbf{10.414}$ |

---

### Step 3: Selection for Round 2

A\* selects the cell with the lowest $f(n)$ from the table to explore next.

* **Cell Selected:** $\mathbf{(2, 2)}$ with $\mathbf{f(n) = 6.414}$.
* **Next Step:** A\* will explore the neighbors of $\mathbf{(2, 2)}$, starting the new $g(n)$ calculation from the current accumulated cost of $\mathbf{1.414}$.
